...
export const MARKETING_TEMPLATES: MarketingTemplateMeta[] = [
  ...
  { id: "dark_confirmation", name: "Dark Confirmation", description: "Premium confirmation layout with a bold hero, details and useful next steps.", icon: "✓", category: "customers", extras: ["items", "highlights", "secondaryCta"], align: "left", font: "modern" },
  { id: "vibrant_offer", name: "Vibrant Offer", description: "Colour-forward promotional story with a hero offer and service highlights.", icon: "%", category: "sales", extras: ["offer", "items", "highlights", "secondaryCta"], align: "center", font: "modern" },
  { id: "travel_catalog", name: "Travel Catalog", description: "Destination-style catalog with a hero, cards and business highlights.", icon: "✈", category: "industry", extras: ["items", "highlights", "secondaryCta"], align: "left", font: "modern" },
  { id: "journey_story", name: "Journey Story", description: "Fresh editorial travel layout with destinations, tips and a featured experience.", icon: "↗", category: "industry", extras: ["items", "highlights", "secondaryCta"], align: "left", font: "modern" },
  { id: "order_thankyou", name: "Order Thank You", description: "Warm post-purchase journey with a hero item and next-step timeline.", icon: "♥", category: "customers", extras: ["items", "highlights", "secondaryCta"], align: "center", font: "modern" },
  { id: "editorial_feature", name: "Editorial Feature", description: "Long-form premium feature with comparison or proof sections.", icon: "Aa", category: "news", extras: ["items", "highlights", "secondaryCta"], align: "left", font: "serif" },
  { id: "pricing_story", name: "Pricing Story", description: "Editorial pricing announcement with offer columns.", icon: "▤", category: "sales", extras: ["offer", "items", "highlights", "secondaryCta"], align: "left", font: "serif" },
  { id: "accommodation_showcase", name: "Accommodation Showcase", description: "Hospitality layout for rooms, amenities, reviews and dining.", icon: "⌂", category: "industry", extras: ["items", "highlights", "secondaryCta"], align: "center", font: "serif" },
  { id: "menu_editorial", name: "Menu Editorial", description: "Restaurant-style menu story with featured dish and item cards.", icon: "✦", category: "industry", extras: ["items", "secondaryCta"], align: "center", font: "serif" },
  { id: "product_catalog", name: "Product Catalog", description: "High-impact catalog with a feature hero and collection cards.", icon: "▦", category: "sales", extras: ["items", "secondaryCta"], align: "center", font: "modern" },
  { id: "dark_menu", name: "Dark Menu", description: "Immersive dark menu with category tiles and a CTA.", icon: "◈", category: "industry", extras: ["items", "secondaryCta"], align: "center", font: "modern" },
  { id: "dark_product", name: "Dark Product Feature", description: "Dark premium product launch with a large hero and feature grid.", icon: "●", category: "sales", extras: ["items", "highlights", "secondaryCta"], align: "center", font: "modern" },
  { id: "product_story", name: "Product Story", description: "Premium product narrative with alternating image and copy sections.", icon: "◐", category: "sales", extras: ["items", "highlights", "secondaryCta"], align: "left", font: "modern" },
  { id: "agency_clean", name: "Agency Clean", description: "Crisp professional service newsletter with portfolio and proof blocks.", icon: "□", category: "industry", extras: ["items", "highlights", "secondaryCta"], align: "left", font: "modern" },
  { id: "beauty_offer", name: "Beauty Offer", description: "Soft premium campaign for beauty, wellness and lifestyle brands.", icon: "✧", category: "industry", extras: ["offer", "items", "highlights", "secondaryCta"], align: "center", font: "modern" },
  { id: "commerce_grid", name: "Commerce Grid", description: "Modern ecommerce sale layout with dense product discovery.", icon: "▥", category: "sales", extras: ["offer", "items", "highlights", "secondaryCta"], align: "left", font: "modern" },
...
function refTopbar(x: Ctx, dark = false) {
  const bg = dark ? x.dark : "#ffffff";
  const fg = dark ? "#ffffff" : x.tx;
  return `<div style="background:${bg};padding:18px 28px;border-bottom:1px solid ${dark ? mix(x.dark,"#ffffff",0.18) : x.line};"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"><tr><td valign="middle">${x.brand.logoUrl ? `<img src="${safeUrl(x.brand.logoUrl)}" alt="${esc(x.brand.name)}" height="30" style="display:block;max-width:160px;height:30px;width:auto;border:0;" />` : `<div style="font-size:18px;font-weight:900;letter-spacing:.4px;color:${fg};">${esc(x.brand.name)}</div>`}</td><td align="right" style="font-size:11px;color:${dark ? mix(fg,x.dark,.25) : x.muted};">${esc(x.brand.businessType ?? "")}</td></tr></table></div>`;
}
function refHero(x: Ctx, radius = 0) { return x.image ? `<div style="background:${x.soft};">${img(x,x.image,x.c.headline || x.brand.name,572,radius)}</div>` : ""; }
function refCards(x: Ctx, items: MarketingItem[], cols = 3) {
  const list = items.slice(0, cols === 3 ? 6 : 4); if (!list.length) return "";
  const rows:string[]=[]; for(let i=0;i<list.length;i+=cols){ const row=list.slice(i,i+cols); rows.push(`<tr>${row.map(item=>`<td valign="top" width="${Math.round(100/cols)}%" style="padding:7px;">${itemCard(x,item,Math.round(572/cols-14),true)}</td>`).join("")}${row.length<cols?Array.from({length:cols-row.length},()=>`<td width="${Math.round(100/cols)}%">&nbsp;</td>`).join(""):""}</tr>`); }
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0 -7px;width:calc(100% + 14px);">${rows.join("")}</table>`;
}
function refFeature(x: Ctx, item: MarketingItem | undefined, reverse = false) {
  if (!item) return ""; const image=item.imageUrl?img(x,item.imageUrl,item.name,270,12):itemPlaceholder(x,item.name,170,270);
  const copy=`<div style="padding:${reverse?"4px 0 4px 22px":"4px 22px 4px 0"};"><div style="font-size:11px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:${x.pt};margin-bottom:8px;">${esc(x.c.eyebrow || x.brand.businessType || "Featured")}</div><div style="font-size:24px;line-height:29px;font-weight:800;color:${x.tx};margin-bottom:8px;">${esc(item.name)}</div>${item.description?`<div style="font-size:13px;line-height:21px;color:${x.muted};">${esc(item.description)}</div>`:""}${item.price?`<div style="margin-top:10px;font-size:15px;font-weight:800;color:${x.pt};">${esc(item.price)}</div>`:""}</div>`;
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 24px;"><tr>${reverse?`<td width="52%" valign="middle">${copy}</td><td width="48%" valign="middle">${image}</td>`:`<td width="48%" valign="middle">${image}</td><td width="52%" valign="middle">${copy}</td>`}</tr></table>`;
}
function refIconRow(x: Ctx, list: string[] | undefined, dark = false) {
  if (!list?.length) return ""; const fg=dark?"#ffffff":x.tx;
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"><tr>${list.slice(0,4).map(t=>`<td align="center" valign="top" style="padding:8px 5px;"><div style="width:38px;height:38px;line-height:38px;border-radius:50%;background:${dark?mix(x.acc,x.dark,.55):x.soft};color:${dark?readableOn(mix(x.acc,x.dark,.55)):x.pt};font-size:14px;font-weight:900;margin:0 auto 7px;">✓</div><div style="font-size:11px;line-height:16px;font-weight:700;color:${fg};">${esc(t)}</div></td>`).join("")}</tr></table>`;
}

const RENDERERS: Record<MarketingTemplateId, (x: Ctx) => string> = {
  dark_confirmation(x) { const {c}=x,D=x.dark,A=x.acc; return `${refTopbar(x,true)}${pad(`${caps(x,c.eyebrow,A)}${h1(x,c.headline,{size:42,color:"#fff",mb:14})}${para(x,c.body,{size:14,lh:23,color:mix("#fff",D,.28),mb:22})}${ctas(x,{bg:A,fg:readableOn(A),align:"left"})}`,{bg:D,top:28,bottom:30,align:"left"})}${refHero(x)}${pad(`${x.items.length?refCards(x,x.items,2):""}${refIconRow(x,c.highlights)}${sign(x,{align:"left"})}`,{top:24,bottom:28,align:"left"})}`; },
  vibrant_offer(x) { const {c}=x,bg=x.p,fg=readableOn(bg); return `${refTopbar(x)}${refHero(x)}${pad(`${c.offerLabel?`<div style="font-size:40px;font-weight:900;color:${fg};margin-bottom:8px;">${esc(c.offerLabel)}</div>`:""}${h1(x,c.headline,{size:27,color:fg})}${para(x,c.body,{color:fg,align:"center",mb:18})}${ctas(x,{bg:fg,fg:bg,align:"center"})}`,{bg,top:28,bottom:30,align:"center"})}${pad(`${c.highlights?.length?refIconRow(x,c.highlights):""}${refCards(x,c.items,2)}${sign(x)}`,{top:28,bottom:30,align:"center"})}`; },
  travel_catalog(x) { const {c}=x; return `${refTopbar(x)}${refHero(x)}${pad(`${plainEyebrow(x,c.eyebrow,x.pt,"left")}${h1(x,c.headline,{size:34,align:"left"})}${para(x,c.body,{align:"left",mb:18})}${ctas(x,{align:"left"})}`,{top:28,bottom:26,align:"left"})}${pad(`${refCards(x,x.items,3)}${c.highlights?.length?refIconRow(x,c.highlights):""}${sign(x)}`,{top:22,bottom:28,align:"left"})}`; },
  journey_story(x) { const {c}=x; return `${refTopbar(x)}${pad(`${caps(x,c.eyebrow,x.pt)}${h1(x,c.headline,{size:34})}${para(x,c.body,{mb:20})}${ctas(x)}`,{top:30,bottom:26,align:"center"})}${pad(`${refFeature(x,x.items[0])}${c.highlights?.length?refIconRow(x,c.highlights):""}${refCards(x,x.items.slice(1),1)}${sign(x)}`,{top:24,bottom:28,align:"left"})}`; },
  order_thankyou(x) { const {c}=x; return `${refTopbar(x)}${pad(`${caps(x,c.eyebrow,x.pt)}${h1(x,c.headline,{size:36})}${para(x,c.body,{mb:20})}${refFeature(x,x.items[0])}${ctas(x)}`,{top:30,bottom:24,align:"center"})}${pad(`${c.highlights?.length?refIconRow(x,c.highlights):""}${sign(x)}`,{top:24,bottom:30,align:"center"})}`; },
  editorial_feature(x) { const {c}=x; return `${refTopbar(x)}${refHero(x)}${pad(`${caps(x,c.eyebrow,x.pt)}${h1(x,c.headline,{size:40,ls:"-1px"})}${para(x,c.body,{size:16,lh:27,mb:24})}${c.highlights?.length?softCards(x,c.highlights):""}`,{top:30,bottom:26,align:"left"})}${pad(`${refFeature(x,x.items[0])}${refFeature(x,x.items[1],true)}${refCards(x,x.items.slice(2),2)}${ctas(x,{align:"left"})}${sign(x,{align:"left"})}`,{top:24,bottom:30,align:"left"})}`; },
  pricing_story(x) { const {c}=x; return `${refTopbar(x)}${pad(`${caps(x,c.eyebrow,x.pt)}${h1(x,c.headline,{size:34,align:"left"})}${para(x,c.body,{align:"left",mb:18})}${c.offerLabel?`<div style="font-size:30px;font-weight:900;color:${x.pt};margin-bottom:18px;">${esc(c.offerLabel)}</div>`:""}${ctas(x,{align:"left"})}`,{top:28,bottom:24,align:"left"})}${pad(`${refCards(x,x.items,2)}${refIconRow(x,c.highlights)}${sign(x,{align:"left"})}`,{top:24,bottom:30,align:"left"})}`; },
  accommodation_showcase(x) { const {c}=x; return `${refTopbar(x)}${refHero(x)}${pad(`${h1(x,c.headline,{size:34})}${para(x,c.body,{mb:20})}${ctas(x)}`,{top:26,bottom:24,align:"center"})}${pad(`${refCards(x,x.items,2)}${refIconRow(x,c.highlights)}${sign(x)}`,{top:24,bottom:30,align:"center"})}`; },
  menu_editorial(x) { const {c}=x; return `${refTopbar(x)}${refHero(x)}${pad(`${c.eyebrow?`<div style="font-size:15px;font-style:italic;color:${x.muted};margin-bottom:8px;">${esc(c.eyebrow)}</div>`:""}${h1(x,c.headline,{size:38})}${para(x,c.body,{size:15,lh:25,mb:22})}${ctas(x)}`,{top:30,bottom:24,align:"center"})}${pad(`${menuList(x,x.items)}${refIconRow(x,c.highlights)}${sign(x)}`,{top:24,bottom:30,align:"center"})}`; },
  product_catalog(x) { const {c}=x; return `${refTopbar(x,true)}${pad(`${h1(x,c.headline,{size:38,color:"#fff"})}${para(x,c.body,{color:mix("#fff",x.dark,.25),mb:18})}${ctas(x,{bg:x.acc,fg:readableOn(x.acc)})}`,{bg:x.dark,top:30,bottom:24,align:"center"})}${refHero(x)}${pad(`${refCards(x,x.items,3)}${ctas(x)}${sign(x)}`,{top:26,bottom:30,align:"center"})}`; },
  dark_menu(x) { const {c}=x,D=x.dark,A=x.acc; return `${refTopbar(x,true)}${pad(`${caps(x,c.eyebrow,A)}${h1(x,c.headline,{size:38,color:"#fff"})}${para(x,c.body,{color:mix("#fff",D,.25),mb:20})}${ctas(x,{bg:A,fg:readableOn(A)})}`,{bg:D,top:30,bottom:26,align:"center"})}${pad(refCards(x,x.items,3),{top:20,bottom:22,bg:D,align:"center"})}${refHero(x)}${pad(`${refIconRow(x,c.highlights,true)}${ctas(x,{bg:A,fg:readableOn(A)})}${sign(x)}`,{top:22,bottom:30,align:"center"})}`; },
  dark_product(x) { const {c}=x,D=x.dark,A=x.acc; return `${refTopbar(x,true)}${pad(`${caps(x,c.eyebrow,A)}${h1(x,c.headline,{size:42,color:"#fff"})}${para(x,c.body,{size:16,lh:25,color:mix("#fff",D,.25),mb:18})}${c.offerLabel?`<div style="font-size:18px;font-weight:800;color:${A};margin-bottom:16px;">${esc(c.offerLabel)}</div>`:""}${ctas(x,{bg:A,fg:readableOn(A)})}`,{bg:D,top:30,bottom:26,align:"center"})}${refHero(x)}${pad(`${refIconRow(x,c.highlights,true)}${refCards(x,x.items,2)}${sign(x)}`,{top:24,bottom:30,bg:D,align:"center"})}`; },
  product_story(x) { const {c}=x; return `${refTopbar(x)}${pad(`${caps(x,c.eyebrow,x.pt)}${h1(x,c.headline,{size:36})}${para(x,c.body,{mb:20})}${ctas(x)}`,{top:30,bottom:24,align:"center"})}${pad(`${refFeature(x,x.items[0])}${refFeature(x,x.items[1],true)}${refCards(x,x.items.slice(2),2)}${refIconRow(x,c.highlights)}${sign(x)}`,{top:24,bottom:30,align:"left"})}`; },
  agency_clean(x) { const {c}=x; return `${refTopbar(x)}${pad(`${caps(x,c.eyebrow,x.pt)}${h1(x,c.headline,{size:36,align:"left"})}${para(x,c.body,{size:15,lh:25,align:"left",mb:18})}${ctas(x,{align:"left"})}`,{top:30,bottom:26,align:"left"})}${refHero(x)}${pad(`${refCards(x,x.items,2)}${c.highlights?.length?softCards(x,c.highlights):""}${sign(x,{align:"left"})}`,{top:20,bottom:30,align:"left"})}`; },
  beauty_offer(x) { const {c}=x,bg=x.soft; return `${refTopbar(x)}${pad(`${caps(x,c.eyebrow,x.pt)}${c.offerLabel?`<div style="font-size:48px;line-height:52px;font-weight:900;color:${x.pt};margin-bottom:8px;">${esc(c.offerLabel)}</div>`:""}${h1(x,c.headline,{size:30})}${para(x,c.body,{mb:18})}${c.couponCode?codeBox(x,{bg:"#fff"}):""}${ctas(x)}`,{bg,top:30,bottom:28,align:"center"})}${refHero(x)}${pad(`${refIconRow(x,c.highlights)}${refCards(x,x.items,3)}${sign(x)}`,{top:24,bottom:30,align:"center"})}`; },
  commerce_grid(x) { const {c}=x,D=x.dark,A=x.acc; return `${refTopbar(x,true)}${pad(`${c.offerLabel?`<div style="font-size:13px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:${A};margin-bottom:8px;">${esc(c.offerLabel)}</div>`:""}${h1(x,c.headline,{size:38,color:"#fff"})}${para(x,c.body,{color:mix("#fff",D,.25),mb:18})}${ctas(x,{bg:A,fg:readableOn(A)})}`,{bg:D,top:30,bottom:26,align:"center"})}${pad(refCards(x,x.items,2),{top:20,bottom:20})}${pad(`${refIconRow(x,c.highlights)}${c.offerNote?`<div style="margin:18px 0;font-size:14px;font-weight:800;color:${x.pt};text-align:center;">${esc(c.offerNote)}</div>`:""}${ctas(x,{bg:A,fg:readableOn(A)})}${sign(x)}`,{top:18,bottom:30,align:"center"})}`; },
  announcement(x) {
    const { c } = x;
...
export function defaultMarketingContent(template: MarketingTemplateId, brand: MarketingBrand, items: MarketingItem[]): MarketingDefaults {
  ...
  switch (template) {
    ...
    case "dark_confirmation": return { ...base, subject: "Your confirmation from " + name, previewText: "Your details and next steps are inside.", eyebrow: "Confirmation", headline: "You're all set.", body: "Thank you for choosing us. Here are the details you need and the next steps to keep things moving.", ctaLabel: buy, items: items.slice(0,4), highlights: ["Confirmation details", "Helpful next steps", "Support when you need it"], imageUrl: hero };
    case "vibrant_offer": return { ...base, subject: "A special offer from " + name, previewText: "A limited-time offer selected for you.", eyebrow: "Special offer", offerLabel: "Save 20%", offerNote: "What's included", headline: "Something worth enjoying.", body: brand.businessDescription ?? "Explore a special selection and enjoy a limited-time offer from our team.", ctaLabel: buy, items: items.slice(0,4), highlights: ["Thoughtfully selected", "Easy to book or buy", "Available for a limited time"], imageUrl: hero };
    case "travel_catalog": return { ...base, subject: "Explore with " + name, previewText: "Featured experiences, offers and places to discover.", eyebrow: "Explore", headline: "Your next experience starts here.", body: brand.businessDescription ?? "Discover featured experiences, places and services selected by our team.", ctaLabel: buy, items: items.slice(0,6), highlights: ["Curated experiences", "Flexible options", "Easy planning"], imageUrl: hero };
    case "journey_story": return { ...base, subject: "Discover more from " + name, previewText: "Ideas, tips and featured experiences.", eyebrow: "Travel notes", headline: "Make the next journey memorable.", body: brand.businessDescription ?? "A fresh collection of ideas, places and experiences to help you plan your next adventure.", ctaLabel: buy, items: items.slice(0,5), highlights: ["Save time", "Plan smart", "Discover locally"], imageUrl: hero };
    case "order_thankyou": return { ...base, subject: "Thank you for choosing " + name, previewText: "Your order or booking is being taken care of.", eyebrow: "Thank you", headline: "We appreciate you.", body: "Your request is in good hands. Here is what happens next and where to find more information.", ctaLabel: "View details", items: items.slice(0,2), highlights: ["We're preparing your request", "You'll receive an update", "We're here if you need help"], signature: "With thanks,\n" + team, imageUrl: itemHero };
    case "editorial_feature": return { ...base, subject: "A closer look from " + name, previewText: "A featured story and selected highlights.", eyebrow: "Featured story", headline: "A closer look at what matters.", body: brand.businessDescription ?? "Explore the story behind a featured offering, plus a few highlights selected for you.", ctaLabel: "Explore more", items: items.slice(0,4), highlights: ["The story behind it", "What makes it useful", "What to explore next"], imageUrl: hero };
    case "pricing_story": return { ...base, subject: "New pricing from " + name, previewText: "Explore the latest options and offers.", eyebrow: "Pricing update", headline: "More ways to choose what fits.", body: "Compare selected options, discover what's included and choose the experience that works for you.", ctaLabel: buy, items: items.slice(0,4), highlights: ["Flexible options", "Clear value", "Easy next steps"], offerLabel: "New season pricing", imageUrl: hero };
    case "accommodation_showcase": return { ...base, subject: "Stay with " + name, previewText: "Rooms, amenities, dining and guest favourites.", eyebrow: "Your stay", headline: "A stay designed around you.", body: brand.businessDescription ?? "Explore accommodation, amenities and experiences available from our team.", ctaLabel: "Explore and book", items: items.slice(0,6), highlights: ["Comfortable spaces", "Dining and experiences", "Helpful service", "Easy reservations"], imageUrl: hero };
    case "menu_editorial": return { ...base, subject: "What's on at " + name, previewText: "Featured dishes, services or selections from our team.", eyebrow: "Featured selection", headline: "Discover what's on offer.", body: brand.businessDescription ?? "Explore featured selections and discover something worth coming back for.", ctaLabel: buy, items: items.slice(0,8), highlights: ["Fresh selection", "Made with care", "Easy to order or reserve"], imageUrl: hero };
    case "product_catalog": return { ...base, subject: "Explore the collection from " + name, previewText: "Featured products and services in one place.", eyebrow: "Featured collection", headline: "Built for what you need next.", body: brand.businessDescription ?? "Explore featured products and services from our latest collection.", ctaLabel: buy, items: items.slice(0,6), imageUrl: itemHero };
    case "dark_menu": return { ...base, subject: "Discover " + name, previewText: "A premium selection curated for you.", eyebrow: "The selection", headline: "Explore the menu.", body: brand.businessDescription ?? "Discover featured products, services or menu selections and choose what fits you.", ctaLabel: buy, items: items.slice(0,9), highlights: ["Featured selection", "Personal service", "Easy reservations"], imageUrl: hero };
    case "dark_product": return { ...base, subject: "Meet something new from " + name, previewText: "A premium feature, designed around your needs.", eyebrow: "New", offerLabel: "Available now", headline: "Designed to stand out.", body: brand.businessDescription ?? "Meet a featured product or service and explore what makes it worth your attention.", ctaLabel: buy, items: items.slice(0,4), highlights: ["Thoughtful design", "Premium experience", "Made for everyday use", "Support when you need it"], imageUrl: itemHero };
    case "product_story": return { ...base, subject: "The story behind " + name, previewText: "A closer look at selected products and services.", eyebrow: "The story", headline: "Made to be experienced.", body: brand.businessDescription ?? "Take a closer look at selected offerings, the thinking behind them and what makes them useful.", ctaLabel: buy, items: items.slice(0,5), highlights: ["Designed with purpose", "Quality you can see", "Simple next steps"], imageUrl: hero };
    case "agency_clean": return { ...base, subject: "A fresh update from " + name, previewText: "Selected work, services and useful updates.", eyebrow: "Studio update", headline: "Clear work. Thoughtful results.", body: brand.businessDescription ?? "A concise look at what we're working on and how we can help.", ctaLabel: "Explore our work", items: items.slice(0,4), highlights: ["Clear communication", "Focused expertise", "Practical outcomes"], imageUrl: hero };
    case "beauty_offer": return { ...base, subject: "A little something from " + name, previewText: "A fresh offer and featured services.", eyebrow: "Treat yourself", offerLabel: "Save 20%", headline: "Feel good, look your best.", body: brand.businessDescription ?? "Explore featured services and enjoy a limited-time offer from our team.", ctaLabel: "Book now", items: items.slice(0,4), highlights: ["Personalised care", "Premium service", "Easy booking"], imageUrl: hero };
    case "commerce_grid": return { ...base, subject: "Deals from " + name, previewText: "Featured picks and limited-time offers.", eyebrow: "Sale", offerLabel: "Limited-time offers", offerNote: "Shop before the offer ends.", headline: "Grab the picks before they're gone.", body: brand.businessDescription ?? "Explore featured products and services, selected for this campaign.", ctaLabel: "Shop now", items: items.slice(0,8), highlights: ["Featured picks", "Limited availability", "Easy checkout"], imageUrl: hero };
    default:
