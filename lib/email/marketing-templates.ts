...
const RENDERERS: Record<MarketingTemplateId, (x: Ctx) => string> = {
  ...
  ref_confirmation(x) { const {c}=x; return `${refHero(x,c.eyebrow,c.headline,c.body,c.ctaLabel,true)}${c.items.length?pad(refTwoCol(x,c.items),{top:22,bottom:18}):""}${c.highlights?.length?pad(featureRows(x,c.highlights,{color:x.acc}),{top:12,bottom:18}):""}${refSocialFooter(x)}`; },
  ref_offer(x) { const {c}=x; return `${x.image?fullBleed(x,c.headline):""}${pad(`${caps(x,c.eyebrow,x.pt,"left")}${c.offerLabel?`<div style="font-size:48px;line-height:52px;font-weight:900;color:${x.pt};text-align:left;margin-bottom:8px;">${esc(c.offerLabel)}</div>`:""}${h1(x,c.headline,{size:30,align:"left"})}${para(x,c.body,{align:"left",mb:18})}${c.couponCode?codeBox(x,{align:"left"}):""}${ctas(x,{align:"left"})}`,{align:"left",bg:x.soft})}${c.highlights?.length?pad(`${refSectionTitle(x,"What’s included")}${featureRows(x,c.highlights,{color:x.pt})}`,{top:8,bottom:18}):""}${c.items.length?pad(`${refSectionTitle(x,"Featured")}${refTwoCol(x,c.items)}`,{top:8,bottom:18}):""}${refSocialFooter(x)}`; },
  ref_catalog(x) { const {c}=x; return `${refHero(x,c.eyebrow,c.headline,c.body,c.ctaLabel,false)}${c.items.length?`${refSectionTitle(x,"Featured selection","Explore the latest products, rooms or services.")}${pad(refTwoCol(x,c.items),{top:8,bottom:20})}`:""}${c.highlights?.length?pad(`${refSectionTitle(x,"Why choose us")}${checkList(x,c.highlights)}`,{top:8,bottom:18}):""}${refSocialFooter(x)}`; },
  ref_journey(x) { const {c}=x; return `${x.image?fullBleed(x,c.headline):""}${pad(`${caps(x,c.eyebrow,x.pt)}${h1(x,c.headline,{size:34})}${para(x,c.body,{mb:20})}${ctas(x)}`,{align:"left"})}${c.highlights?.length?pad(`${refSectionTitle(x,"Travel tips & insights")}${refTwoCol(x,c.highlights.map(t=>({name:t,description:"Helpful guidance for your customers."})))}`,{top:8,bottom:18}):""}${c.items.length?`${refSectionTitle(x,"Popular selection")}${pad(refTwoCol(x,c.items),{top:8,bottom:20})}`:""}${refSocialFooter(x)}`; },
  ref_thankyou(x) { const {c}=x; const steps=(c.highlights??[]).slice(0,4); return `${pad(`${c.eyebrow?plainEyebrow(x,c.eyebrow,x.pt,"center"):""}${h1(x,c.headline,{size:32})}${para(x,c.body,{align:"center",mb:22})}${x.image?`<div style="margin:0 auto 24px;max-width:420px;">${img(x,x.image,c.headline,420)}</div>`:""}${ctas(x)}`,{align:"center"})}${steps.length?pad(`${refSectionTitle(x,"Here’s what happens next")}${steps.map((t,i)=>`<div style="padding:12px 0;text-align:center;border-bottom:1px solid ${x.line};"><div style="margin:0 auto 7px;width:30px;height:30px;line-height:30px;border-radius:50%;background:${x.soft};color:${x.pt};font-weight:900;">${i+1}</div><div style="font-size:14px;font-weight:700;color:${x.tx};">${esc(t)}</div></div>`).join("")}`,{top:8,bottom:18,align:"center"}):""}${refSocialFooter(x)}`; },
  ref_editorial(x) { const {c}=x; return `${x.image?fullBleed(x,c.headline):""}${pad(`${caps(x,c.eyebrow,x.pt,"left")}${h1(x,c.headline,{size:38,align:"left"})}${para(x,c.body,{size:16,lh:27,align:"left",mb:20})}${ctas(x,{align:"left"})}`,{align:"left"})}${c.highlights?.length?pad(`${refSectionTitle(x,"The story")}${softCards(x,c.highlights)}`,{top:8,bottom:18}):""}${c.items.length?`${refSectionTitle(x,"Explore the collection")}${pad(refTwoCol(x,c.items),{top:8,bottom:20})}`:""}${refSocialFooter(x)}`; },
  ref_pricing(x) { const {c}=x; return `${pad(`${caps(x,c.eyebrow,x.pt,"left")}${h1(x,c.headline,{size:34,align:"left"})}${para(x,c.body,{align:"left"})}`,{align:"left",bg:x.soft})}<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"><tr><td width="50%" valign="top" style="padding:24px 14px 24px 34px;border-right:1px solid ${x.line};"><div style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:${x.muted};">${esc(c.eyebrow||"Standard")}</div><div style="font-size:28px;font-weight:900;color:${x.pt};margin-top:6px;">${esc(c.offerLabel||"Current rate")}</div></td><td width="50%" valign="top" style="padding:24px 34px 24px 14px;"><div style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:${x.muted};">${esc(c.offerNote||"Flexible option")}</div><div style="font-size:28px;font-weight:900;color:${x.pt};margin-top:6px;">${esc(c.couponCode||"Available now")}</div></td></tr></table>${c.highlights?.length?pad(featureRows(x,c.highlights,{color:x.pt}),{top:8,bottom:18}):""}${pad(ctas(x,{align:"left"}),{top:8,bottom:22,align:"left"})}${refSocialFooter(x)}`; },
  ref_hotel(x) { const {c}=x; return `${x.image?fullBleed(x,c.headline):""}${pad(`${h1(x,c.headline,{size:32,align:"left"})}${para(x,c.body,{align:"left"})}${ctas(x,{align:"left"})}`,{align:"left"})}${c.items.length?`${refSectionTitle(x,"Our accommodations")}${pad(refTwoCol(x,c.items),{top:8,bottom:18})}`:""}${c.highlights?.length?pad(`${refSectionTitle(x,"Why guests choose us")}${checkList(x,c.highlights)}`,{top:8,bottom:18}):""}${refSocialFooter(x)}`; },
  ref_restaurant(x) { const {c}=x; return `${x.image?fullBleed(x,c.headline):""}${pad(`${caps(x,c.eyebrow,x.pt)}${h1(x,c.headline,{size:34})}${para(x,c.body,{mb:18})}${ctas(x)}`,{align:"center"})}${c.items.length?`${refSectionTitle(x,"Featured menu")}${pad(refTwoCol(x,c.items),{top:8,bottom:18})}`:""}${c.highlights?.length?pad(`${refSectionTitle(x,"About the experience")}${para(x,c.highlights.join("\n\n"),{align:"left"})}`,{top:8,bottom:18,align:"left"}):""}${refSocialFooter(x)}`; },
  ref_food_catalog(x) { const {c}=x; return `${refHero(x,c.eyebrow,c.headline,c.body,c.ctaLabel,false)}${c.items.length?`${refSectionTitle(x,"Menu selection")}${pad(refTwoCol(x,c.items),{top:8,bottom:20})}`:""}${refSocialFooter(x)}`; },
  ref_dark_menu(x) { const {c}=x; const D=x.dark; return `<div style="background:${D};padding:28px 24px 10px;">${c.eyebrow?`<div style="text-align:center;color:${x.acc};font-size:11px;letter-spacing:3px;text-transform:uppercase;font-weight:800;">${esc(c.eyebrow)}</div>`:""}${h1(x,c.headline,{size:36,color:"#fff",align:"center",mb:12})}${para(x,c.body,{color:"#fff",align:"center",mb:20})}${c.items.length?`<table role="presentation" width="100%" cellspacing="8" cellpadding="0" border="0">${c.items.slice(0,9).map(i=>`<tr><td style="background:#111;padding:18px;text-align:center;">${i.imageUrl?`<img src="${safeUrl(i.imageUrl)}" alt="${esc(i.name)}" width="170" style="width:100%;max-width:170px;height:110px;object-fit:cover;display:block;margin:0 auto 10px;">`:""}<div style="color:#fff;font-size:14px;font-weight:800;">${esc(i.name)}</div>${i.price?`<div style="color:${x.acc};font-size:12px;font-weight:800;margin-top:5px;">${esc(i.price)}</div>`:""}</td></tr>`).join("")}</table>`:""}${ctas(x,{bg:x.acc,fg:readableOn(x.acc)})}</div>${refSocialFooter(x)}`; },
  ref_product_launch(x) { const {c}=x; return `${pad(`${caps(x,c.eyebrow,x.pt,"center")}${h1(x,c.headline,{size:38})}${para(x,c.body,{size:16,lh:26,mb:20})}${ctas(x)}`,{align:"center",bg:x.dark})}${x.image?fullBleed(x,c.headline):""}${c.highlights?.length?pad(`${refSectionTitle(x,"Why it matters")}${featureRows(x,c.highlights,{color:x.pt})}`,{top:8,bottom:18}):""}${c.items.length?`${refSectionTitle(x,"Explore")}${pad(refTwoCol(x,c.items),{top:8,bottom:20})}`:""}${refSocialFooter(x)}`; },
