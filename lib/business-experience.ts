...
export function getBusinessExperience(category?: string | null): BusinessExperience {
  ...
  if (SERVICE.includes(c)) {
    ...
    return {
      ...
      preferredSections: hotel ? ["hero", "gallery", "categories", "catalog", "features", "stats", "map", "testimonials", "contact"] : restaurant ? ["hero", "categories", "catalog", "gallery", "features", "map", "testimonials", "contact"] : portfolio ? ["hero", "gallery", "features", "about", "testimonials", "contact"] : ["hero", "catalog", "stats", "features", "testimonials", "map", "contact"],
    };
  }
  ...
  return {
    ...
    preferredSections: ["hero", "categories", "catalog", "text", "features", "testimonials", "newsletter", "contact"],
  };
}
...
export function buildIndustryHomepage(category: string | null | undefined, storeName: string, description?: string | null, heroImage?: string | null): BuilderConfig {
  ...
  const sections = experience.preferredSections.map((type, index) => {
    ...
    if (type === "stats") return section("stats", type, { eyebrow: "Availability", heading: experience.primaryAction, padding: "normal" });
    ...
    if (type === "text") return section("text", type, { eyebrow: "Special offer", heading: experience.mode === "commerce" ? "Good things are waiting" : "Book with confidence", body: "Add your current promotion, launch offer or seasonal message here.", ctaLabel: experience.primaryAction, ctaHref: "#catalog", padding: "normal" });
