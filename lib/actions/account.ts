...
export async function addAddress(input: {
  ...
  if (session.user.role === "CUSTOMER") {
    return {
      success: false,
      error: "Customer accounts must use a store account.",
    };
  }
...
export async function deleteAddress(
  ...
  if (session.user.role === "CUSTOMER") {
    return {
      success: false,
      error: "Customer accounts must use a store-scoped account.",
    };
  }
...
export async function toggleWishlist(input: {
  ...
  if (session.user.role === "CUSTOMER") {
    ...
    if (!membership) {
      return {
        success: false,
        error: "This customer account belongs to another store.",
      };
    }
  }
...
export async function saveCart(
  ...
  if (session.user.role === "CUSTOMER") {
    ...
    if (!membership) {
      return {
        success: false,
        error: "This customer account does not belong to that store.",
      };
    }
  }
...
// and new customer-facing model in one
// call.
...
export async function getAccountOverviewForStore(
  storeSlug: string
) {
  const session = await auth();
...
async function getStoreCustomerContext(
  storeSlug: string
) {
  return requireStoreCustomer(storeSlug);
}

export async function getStoreCustomerOverview(
  storeSlug: string
) {
