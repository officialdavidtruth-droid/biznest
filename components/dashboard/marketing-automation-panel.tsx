...
export function MarketingAutomationPanel({slug,activeCount}:{slug:string;activeCount:number}){
 ...
 const install=(p:typeof presets[number])=>start(async()=>{const r=await createAutomation(slug,{name:p.name,description:p.description,trigger:p.trigger,actions:p.actions});if(!r.success)alert(r.error||"Could not create automation.");else location.reload()});
