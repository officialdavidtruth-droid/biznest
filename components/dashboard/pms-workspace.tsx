...
export function PmsWorkspace({ slug, storeName, rooms: initialRooms, guests: initialGuests, reservations: initialReservations, cards: initialCards }: PmsWorkspaceProps) {
  ...
  return (
    <div className="flex min-h-screen bg-[#2b1a12] text-white">
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-white/10 bg-[#3b2418] transition-transform lg:static lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <section className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 flex h-20 items-center gap-3 border-b border-white/10 bg-[#2b1a12]/95 px-4 backdrop-blur md:px-7">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {maintenanceRooms.map((r: any) => { const info = notes[r.id]; return <div key={r.id} className="rounded-2xl border border-white/10 bg-black/10 p-5">
          ...
          <div className="mt-5 grid grid-cols-2 gap-2">
            <button onClick={() => onResolve(r, "DIRTY")} className="rounded-xl bg-emerald-400 px-3 py-2 text-xs font-semibold text-[#24150f]">Resolve → Clean</button>
...
function Modal({ title, onClose, children }: any) { return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm"><div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#4f3021] p-5 shadow-2xl"><div className="mb-5 flex items-center justify-between"><h3 className="text-lg font-semibold">{title}</h3><button onClick={onClose} className="rounded-lg p-2 text-white/45 hover:bg-white/5 hover:text-white"><X className="h-4 w-4" /></button></div>{children}</div></div>; }
...
function SelectField({ label, value, onChange, options }: any) { return <label className="block"><span className="mb-1.5 block text-xs text-white/50">{label}</span><select required value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-xl border border-white/10 bg-black/20 px-3 py-3 text-sm text-white"><option value="" className="bg-[#4f3021]">Select {label.toLowerCase()}</option>{options.map(([id, name]: any) => <option key={id} value={id} className="bg-[#4f3021]">{name}</option>)}</select></label>; }