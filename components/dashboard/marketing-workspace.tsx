...
export function MarketingWorkspace({
  ...
  const count: Partial<Record<MarketingTab, number>> = {
    audience: stats.active,
    campaigns: campaigns.length,
    automations: stats.automations,
  };

  const deliveryRate = stats.sentCampaigns
    ? Math.round(
        (stats.delivered /
          Math.max(stats.sentCampaigns * Math.max(stats.active, 1), 1)) *
          100,
      )
    : 0;
  ...
  const statCards: Array<{
  ...
  }> = [
    {
      ...
      trend: stats.unsubscribed
        ? { dir: "down", label: `${stats.unsubscribed} unsubscribed` }
        : undefined,
    },
    {
    ...
    },
    {
      ...
      trend: deliveryRate
        ? { dir: "up", label: `${deliveryRate}% delivery rate` }
        : undefined,
    },
    {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
            Design branded emails, grow your audience and automate what happens next.
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <Link
            ...
            className="inline-flex items-center gap-1.5 rounded-lg border bg-background px-3 py-2 text-xs font-medium hover:border-primary"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3.5 py-2 text-xs font-semibold text-primary-foreground shadow-sm hover:opacity-90"
          >
      ...
      <div className="flex flex-col gap-6 lg:flex-row">
        <nav
          ...
          className="flex gap-1 overflow-x-auto rounded-2xl border bg-background p-1.5 lg:w-56 lg:shrink-0 lg:flex-col lg:gap-1 lg:overflow-visible"
        >
          {TABS.map((t) => {
            const Icon = t.icon;
        ...
        <div className="min-w-0 flex-1">
          <div
            role="tabpanel"
            id="mk-pane-compose"
            aria-labelledby="mk-tab-compose"
            hidden={tab !== "compose"}
          >
          ...
          <div
            role="tabpanel"
            id="mk-pane-audience"
            aria-labelledby="mk-tab-audience"
            hidden={tab !== "audience"}
          >
            <MarketingAudience
              slug={slug}
              subscribers={subscribers}
              total={stats.active + stats.unsubscribed}
          ...
          <div
            role="tabpanel"
            id="mk-pane-campaigns"
            aria-labelledby="mk-tab-campaigns"
          ...
          <div
            role="tabpanel"
            id="mk-pane-automations"
            aria-labelledby="mk-tab-automations"
            hidden={tab !== "automations"}
          >
            <MarketingAutomationPanel
              slug={slug}
              activeCount={stats.automations}
            />
          </div>
        </div>
      </div>
