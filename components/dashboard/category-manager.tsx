...
export function CategoryManager({ slug, initialCategories }: { slug: string; initialCategories: Category[] }) {
  ...
  return <div className="space-y-6">
    <div className="rounded-2xl border bg-card p-4 shadow-sm">
      <div className="mb-4 flex items-center gap-2 font-medium"><Plus className="h-4 w-4" /> Add Category</div>
      <div className="grid gap-3 md:grid-cols-[1fr_180px_1fr_auto]">
        <input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Rooms" className="h-10 rounded-lg border bg-background px-3 text-sm" />
        <select value={type} onChange={e=>setType(e.target.value as any)} className="h-10 rounded-lg border bg-background px-3 text-sm"><option value="PRODUCT">Product</option><option value="SERVICE">Service</option></select>
        <select value={parentId} onChange={e=>setParentId(e.target.value)} className="h-10 rounded-lg border bg-background px-3 text-sm"><option value="">Top-level category</option>{top.filter(c=>c.type===type).map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select>
        <button onClick={add} disabled={!name.trim() || saving} className="h-10 rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground disabled:opacity-50">{saving ? "Saving…" : "Add"}</button>
      </div>
    </div>
    ...
    <div className="grid gap-4 md:grid-cols-2">
      {top.map(c => <div key={c.id} className="rounded-2xl border bg-card p-5 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3"><div className="rounded-xl bg-muted p-2"><FolderTree className="h-5 w-5"/></div><div><h2 className="font-semibold">{c.name}</h2><p className="text-xs text-muted-foreground">{children(c.id).length} subcategories · {c.type === "PRODUCT" ? "Products" : "Services"}</p></div></div>
