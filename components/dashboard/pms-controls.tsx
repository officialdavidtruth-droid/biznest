...
import { createRoom, createGuest, createReservation, updateRoomStatus, checkInReservation, checkOutReservation, setGuestPresence } from "@/lib/actions/pms";
...
export function PmsControls({slug,rooms,guests,reservations}:{slug:string;rooms:any[];guests:any[];reservations:any[]}) {
 ...
 return <div className="space-y-6">
  ...
  <div className="rounded-2xl border p-4"><h2 className="mb-4 font-semibold">Reservations</h2><div className="space-y-3">{reservations.map(r=><div key={r.id} className="flex flex-col gap-3 rounded-xl border p-3 md:flex-row md:items-center md:justify-between"><div><div className="font-medium">{r.guest.fullName} · {r.room.name}</div><div className="text-xs text-muted-foreground">{new Date(r.checkIn).toLocaleString()} → {new Date(r.checkOut).toLocaleString()} · {r.status} · Guest {r.guestPresence}</div></div><div className="flex flex-wrap gap-2">{["PENDING","CONFIRMED"].includes(r.status)&&<button className="btn" onClick={()=>run(()=>checkInReservation(slug,r.id))}>Check in</button>}{r.status==="CHECKED_IN"&&<><button className="btn" onClick={()=>run(()=>setGuestPresence(slug,r.id,r.guestPresence==="IN"?"OUT":"IN"))}>{r.guestPresence==="IN"?"Mark OUT":"Mark IN"}</button><button className="btn" onClick={()=>run(()=>checkOutReservation(slug,r.id))}>Check out</button></>}</div></div>)}</div></div>
  <style jsx>{`.input{width:100%;border:1px solid hsl(var(--border));border-radius:.6rem;padding:.55rem .7rem;background:hsl(var(--background));font-size:.875rem}.btn{border-radius:.6rem;background:hsl(var(--primary));color:hsl(var(--primary-foreground));padding:.55rem .8rem;font-size:.8rem;font-weight:600}`}</style>
 </div>
}
