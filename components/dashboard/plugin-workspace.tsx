...
export function PluginWorkspace({ slug, data }: { slug: string; data: Awaited<ReturnType<typeof getPluginWorkspace>> & { error?: string } }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-xl border bg-background p-4 shadow-sm"><div className="flex items-center gap-2"><BarChart3 className="h-4 w-4 text-primary"/><h2 className="text-sm font-bold">Operational controls</h2></div><div className="mt-3 grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">{data.actions.map(a=><Link key={a.href} href={`/store/${slug}/admin${a.href}`} className="group rounded-lg border p-3 hover:border-primary/40"><p className="text-sm font-bold">{a.label}</p><span className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-primary">Open <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-0.5"/></span></Link>)}</div></section>
