...
function Header({store,slug,active,accent}:{store:any;slug:string;active:string;accent:string}){
 ...
 return <header className="bn-universal-header"><div className="bn-universal-nav">
   ...
   <div className="bn-universal-actions"><Link href={`/store/${slug}/search`} aria-label="Search"><Search size={20}/></Link><CartLink storeSlug={slug} accent={accent} onAccent="#fff" ink="#111"/><AccountLink storeSlug={slug} ink="#111"/><Link href={cta.href} className="bn-universal-quote" style={{background:accent}}>{cta.label} <ArrowRight size={15}/></Link></div>
 </div></header>
}
...
function Hero({store,slug,active,copy,accent,hero,children}:{store:any;slug:string;active:string;copy:any;accent:string;hero:string|null;children:React.ReactNode}){
 return <section className="bn-universal-hero" style={hero?{backgroundImage:`linear-gradient(90deg,rgba(4,13,23,.94) 0%,rgba(4,13,23,.76) 46%,rgba(4,13,23,.22) 100%),url(${hero})`}:{backgroundImage:"radial-gradient(circle at 78% 42%,rgba(20,115,234,.34),transparent 28%),linear-gradient(135deg,#061525 0%,#071522 52%,#102b46 100%)"}}><div className="bn-universal-hero-inner"><div className="bn-universal-eyebrow">{copy.eyebrow}</div><h1>{children}</h1><p>{copy.description}</p><div className="bn-universal-hero-actions"><Link href={primaryCta(store,slug,theme).href} className="bn-primary" style={{background:accent}}>{primaryCta(store,slug,theme).label} <ArrowRight size={16}/></Link><Link href={secondaryCta(store,slug).href} className="bn-secondary">{secondaryCta(store,slug).label} <ArrowRight size={16}/></Link></div><div className="bn-hero-trust"><span><ShieldCheck size={20}/>Quality Service</span><span><Clock3 size={20}/>Fast Response</span><span><Sparkles size={20}/>Professional Results</span></div></div></section>
}
