...
export function SignatureStorefront(props: Props) {
  ...
  const nav = (
    <>
      <header style={{ position: "sticky", top: 0, zIndex: 30, backdropFilter: "blur(18px)", background: `${bg}eF`, borderBottom: `1px solid ${theme.border || ink + "18"}` }}>
        <div className="bn-header-inner" style={{ maxWidth: 1240, margin: "0 auto", padding: "18px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20 }}>
          ...
          <label htmlFor={`bn-nav-${slug}-signature`} className="bn-hamburger" style={{ color: ink }} aria-label="Menu">&#9776;</label>
          <nav className="bn-nav-links" style={{ display: "flex", alignItems: "center", gap: 20, fontSize: 13 }}>
  ...
  const intro = (
    <section style={{ padding: "80px 24px", background: mode === "harvest" ? theme.card : bg, color: ink }}>
      <div className="bn-2col" style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "minmax(0,1.1fr) minmax(280px,.9fr)", gap: 70, alignItems: "start" }}>
  ...
  const catalog = (
    <section style={{ padding: "82px 24px", background: theme.surfaceDark || (mode === "kinetic" ? "#111" : theme.card), color: mode === "kinetic" ? "#fff" : ink }}>
      <div style={{ maxWidth: 1240, margin: "0 auto" }}>
        ...
        <div className="bn-grid-2" style={{ display: "grid", gridTemplateColumns: mode === "kinetic" ? "repeat(3,1fr)" : "repeat(auto-fit,minmax(220px,1fr))", gap: mode === "atelier" ? 26 : 16 }}>
