...
// The tool (contact import + dedupe) is gated: signed-out visitors must sign
// up for Marketing specifically, and signed-in users need an active
// marketing-only store -- a Business Mogul store's plan grants nothing here.
...
export default async function MarketingStandalonePage() {
  ...
  if (access.status === "active") templates = await loadTemplateData(access.storeSlug);
    ...
    <main className="min-h-screen bg-slate-950 text-white">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5">
          ...
          <nav className="flex items-center gap-5 text-sm font-medium">
            ...
            {access.status === "active" ? (
              <Link href="/store/marketing" className="rounded-full bg-orange-500 px-5 py-2.5 font-bold text-slate-950 hover:bg-orange-400">
                Go to workspace
              </Link>
      ...
      <section className="bg-gradient-to-br from-slate-900 via-blue-950 to-slate-950">
        <div className="mx-auto grid max-w-6xl gap-10 px-5 py-16 md:grid-cols-2 md:items-center md:py-24">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.22em] text-orange-400">BizNest Marketing</p>
            ...
            <p className="mt-6 max-w-xl text-lg leading-8 text-slate-200">Organize contacts, prepare campaigns, and review results &mdash; on its own standalone Marketing subscription.</p>
          ...
          <div className="rounded-3xl border border-blue-800 bg-slate-900/80 p-6 shadow-2xl">
            <div className="mb-5 flex items-center justify-between"><span className="font-bold text-white">Marketing workspace</span><span className="rounded-full bg-orange-500/15 px-3 py-1 text-xs font-semibold text-orange-300">{access.status === "active" ? "Active" : "Preview"}</span></div>
