      ...
      <div className="mb-8 overflow-hidden rounded-lg border bg-background">
        <table className="w-full text-sm">
          <thead className="border-b bg-muted/40 text-left text-xs uppercase text-muted-foreground">
            <tr><th className="px-4 py-2">Service</th><th className="px-4 py-2">Price</th><th className="px-4 py-2">Booking</th><th className="px-4 py-2">Status</th></tr>
          </thead>
          <tbody>
            {services.map((s) => (
            ...
            {services.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-muted-foreground">No services yet — add your first one above.</td></tr>
            )}
          </tbody>
        </table>
      </div>
