...
export default async function HotelSectionPage({ params }: { params: Promise<{ slug: string; section: string }> }) {
  ...
  if (section === "rooms") return shell(<>
    ...
    <section style={{ padding: "70px 28px 130px" }}><div style={{ maxWidth: 1240, margin: "0 auto" }}><div className="bn-grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 22 }}>{roomItems.length ? roomItems.map((room, i) => <Link key={room.id} href={`/${slug}/${room.kind}/${room.id}`} style={{ color: "#fff", textDecoration: "none", background: dark, minHeight: 430, position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", inset: 0, background: room.image ? `linear-gradient(180deg, transparent 35%, rgba(0,0,0,.82)), url(${room.image}) center/cover` : `linear-gradient(135deg, ${theme.accent}, ${dark})` }} /><div style={{ position: "relative", minHeight: 430, padding: 28, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 12 }}><span style={{ color: theme.accentSoft || theme.accent, fontSize: 10, letterSpacing: ".16em", textTransform: "uppercase" }}>{room.categoryName || "Accommodation"}</span>{room.price > 0 && <span style={{ fontSize: 13, fontWeight: 800 }}>{formatMoney(room.price, room.currency)}/night</span>}</div><h2 style={{ fontFamily: theme.headlineFont, fontSize: 34, margin: "9px 0 8px" }}>{room.name}</h2><p style={{ margin: 0, maxWidth: 540, color: "rgba(255,255,255,.72)", lineHeight: 1.7, fontSize: 13 }}>{room.description || "Discover this accommodation."}</p><span style={{ marginTop: 18, fontSize: 12, fontWeight: 800 }}>View room <ArrowUpRight size={13} style={{ verticalAlign: "middle" }} /></span></div></Link>) : <p style={{ color: muted }}>Published rooms will appear here once they are added.</p>}</div></div></section>
  </>);
  ...
  if (section === "experience") return shell(<>
    ...
    <section style={{ padding: "70px 28px 130px" }}><div style={{ maxWidth: 1240, margin: "0 auto" }}><div className="bn-grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 20 }}>{experiences.length ? experiences.map((service, i) => <Link key={service.id} href={`/${slug}/service/${service.id}`} style={{ color: theme.ink, textDecoration: "none", borderTop: `1px solid ${theme.border || `${theme.ink}18`}`, paddingTop: 20 }}><div style={{ aspectRatio: "16/10", background: service.image ? `url(${service.image}) center/cover` : `${theme.accent}18`, marginBottom: 20 }} /><span style={{ color: theme.accent, fontSize: 10, fontWeight: 800, letterSpacing: ".16em" }}>0{i + 1}</span><h2 style={{ fontFamily: theme.headlineFont, fontSize: 25, margin: "10px 0 8px" }}>{service.name}</h2><p style={{ color: muted, lineHeight: 1.75, fontSize: 13 }}>{service.description || "Discover more about this experience."}</p></Link>) : <p style={{ color: muted }}>Published experiences will appear here once they are added.</p>}</div></div></section>
  </>);
