  ...
  return (
    <header style={{ position: "sticky", top: 0, zIndex: 50, background: `${theme.bg}F2`, backdropFilter: "blur(18px)", borderBottom: `1px solid ${theme.border || `${theme.ink}18`}` }}>
      <div className="bn-header-inner" style={{ maxWidth: 1320, margin: "0 auto", minHeight: 76, padding: "0 28px", display: "flex", alignItems: "center", gap: 28 }}>
        <Link href={`/${slug}`} style={{ color: theme.ink, textDecoration: "none", display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
  ...
  return (
    <footer style={{ padding: "30px 28px", background: theme.surfaceDark || "#171411", color: "rgba(255,255,255,.62)" }}>
      <div style={{ maxWidth: 1240, margin: "0 auto", display: "flex", justifyContent: "space-between", gap: 20, flexWrap: "wrap", fontSize: 11 }}>
        <Link href={`/${slug}`} style={{ color: "rgba(255,255,255,.75)", textDecoration: "none" }}>© {new Date().getFullYear()} {store.name}</Link>
        <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
          <Link href={`/${slug}/hotel/story`} style={{ color: "inherit", textDecoration: "none" }}>The Hotel</Link>
          <Link href={`/${slug}/hotel/rooms`} style={{ color: "inherit", textDecoration: "none" }}>Rooms</Link>
          <Link href={`/${slug}/hotel/contact`} style={{ color: "inherit", textDecoration: "none" }}>Contact</Link>
        </div>
      </div>
    </footer>
  );
...
export default async function HotelSectionPage({ params }: { params: Promise<{ slug: string; section: string }> }) {
  ...
  if (section === "story") return shell(<>
    <section style={{ padding: "120px 28px 90px" }}><div style={{ maxWidth: 1240, margin: "0 auto" }}>{sectionTitle(theme, "The property", "A place with a point of view.", store.business.description || theme.sub)}</div></section>
  ...
  if (section === "rooms") return shell(<section style={{ padding: "110px 28px 130px" }}><div style={{ maxWidth: 1240, margin: "0 auto" }}>{sectionTitle(theme, "Rooms & suites", "Spaces made for staying well.", "Explore the accommodation collection and choose the room that fits your stay.")}<div className="bn-grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 22, marginTop: 58 }}>{roomItems.length ? roomItems.map((room, i) => <Link key={room.id} href={`/${slug}/${room.kind}/${room.id}`} style={{ color: "#fff", textDecoration: "none", background: dark, minHeight: 430, position: "relative", overflow: "hidden" }}><div style={{ position: "absolute", inset: 0, background: room.image ? `linear-gradient(180deg, transparent 35%, rgba(0,0,0,.82)), url(${room.image}) center/cover` : `linear-gradient(135deg, ${theme.accent}, ${dark})` }} /><div style={{ position: "relative", minHeight: 430, padding: 28, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}><span style={{ color: theme.accentSoft || theme.accent, fontSize: 10, letterSpacing: ".16em", textTransform: "uppercase" }}>{room.categoryName || "Accommodation"}</span><h2 style={{ fontFamily: theme.headlineFont, fontSize: 34, margin: "9px 0 8px" }}>{room.name}</h2><p style={{ margin: 0, maxWidth: 540, color: "rgba(255,255,255,.72)", lineHeight: 1.7, fontSize: 13 }}>{room.description || "Discover this accommodation."}</p><span style={{ marginTop: 18, fontSize: 12, fontWeight: 800 }}>View room <ArrowUpRight size={13} style={{ verticalAlign: "middle" }} /></span></div></Link>) : <p style={{ color: muted }}>Published rooms will appear here once they are added.</p>}</div></div></section>);

  if (section === "experience") return shell(<section style={{ padding: "110px 28px 130px" }}><div style={{ maxWidth: 1240, margin: "0 auto" }}>{sectionTitle(theme, "The experience", "More than a room.", "Discover the services, experiences and moments that shape the stay.")}<div className="bn-grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 20, marginTop: 58 }}>{experiences.length ? experiences.map((service, i) => <Link key={service.id} href={`/${slug}/service/${service.id}`} style={{ color: theme.ink, textDecoration: "none", borderTop: `1px solid ${theme.border || `${theme.ink}18`}`, paddingTop: 20 }}><div style={{ aspectRatio: "16/10", background: service.image ? `url(${service.image}) center/cover` : `${theme.accent}18`, marginBottom: 20 }} /><span style={{ color: theme.accent, fontSize: 10, fontWeight: 800, letterSpacing: ".16em" }}>0{i + 1}</span><h2 style={{ fontFamily: theme.headlineFont, fontSize: 25, margin: "10px 0 8px" }}>{service.name}</h2><p style={{ color: muted, lineHeight: 1.75, fontSize: 13 }}>{service.description || "Discover more about this experience."}</p></Link>) : <p style={{ color: muted }}>Published experiences will appear here once they are added.</p>}</div></div></section>);

  if (section === "gallery") return shell(<section style={{ padding: "110px 28px 130px" }}><div style={{ maxWidth: 1240, margin: "0 auto" }}>{sectionTitle(theme, galleryContent?.eyebrow || "Visual narrative", galleryContent?.title || "See the place before you arrive.", galleryContent?.intro)}{galleryContent?.albums.length ? <div style={{ display: "grid", gap: 90, marginTop: 58 }}>{galleryContent.albums.map((album) => <section key={album.id}><div style={{ maxWidth: 760, marginBottom: 28 }}><h2 style={{ fontFamily: theme.headlineFont, fontSize: 36, margin: 0 }}>{album.title}</h2>{album.description && <p style={{ color: muted, lineHeight: 1.8, fontSize: 14, marginTop: 10 }}>{album.description}</p>}</div><div style={{ display: "grid", gridTemplateColumns: "1.4fr .8fr .8fr", gridAutoRows: 230, gap: 10 }}>{album.images.map((image, i) => <article key={image.id} style={{ gridRow: i === 0 ? "span 2" : undefined, position: "relative", overflow: "hidden", background: theme.card }}><img src={image.image} alt={image.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} /><div style={{ position: "absolute", left: 0, right: 0, bottom: 0, padding: "50px 18px 16px", background: "linear-gradient(transparent, rgba(0,0,0,.75))", color: "#fff" }}><strong style={{ fontFamily: theme.headlineFont, fontSize: 18 }}>{image.title}</strong>{image.caption && <div style={{ fontSize: 11, opacity: .75, marginTop: 4 }}>{image.caption}</div>}</div></article>)}</div></section>)}</div> : <div style={{ display: "grid", gridTemplateColumns: "1.4fr .8fr .8fr", gridAutoRows: 260, gap: 12, marginTop: 55 }}>{gallery.length ? gallery.map((image, i) => <div key={image} style={{ gridRow: i === 0 ? "span 2" : undefined, background: `url(${image}) center/cover` }} />) : <p style={{ color: muted }}>Gallery images will appear here once they are added.</p>}</div>}</div></section>);

  return shell(<>
    <section style={{ padding: "110px 28px 70px" }}><div style={{ maxWidth: 1240, margin: "0 auto" }}>{sectionTitle(theme, "Reservations & concierge", "Let's plan your stay.", "For reservations, questions or special requests, contact the hotel directly.")}<div className="bn-2col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 70, marginTop: 65 } as React.CSSProperties}><div style={{ background: dark, color: "#fff", padding: 40 }}><h2 style={{ fontFamily: theme.headlineFont, fontSize: 34, margin: 0 }}>Your stay starts here.</h2><p style={{ color: "rgba(255,255,255,.65)", lineHeight: 1.8, fontSize: 14 }}>Explore our rooms or reach the concierge team directly.</p><Link href={`/${slug}/hotel/rooms`} style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "#fff", color: "#111", padding: "13px 17px", textDecoration: "none", borderRadius: theme.radius, fontWeight: 800, fontSize: 12 }}>Explore rooms <ArrowUpRight size={14} /></Link></div><div style={{ display: "grid", gap: 22, alignContent: "center" }}>{location && <div style={{ display: "flex", gap: 12, alignItems: "flex-start", fontSize: 14 }}><MapPin size={18} color={theme.accent} />{location}</div>}{store.contactPhone && <a href={`tel:${store.contactPhone}`} style={{ display: "flex", gap: 12, color: theme.ink, textDecoration: "none", fontSize: 14 }}><Phone size={18} color={theme.accent} />{store.contactPhone}</a>}{store.contactEmail && <a href={`mailto:${store.contactEmail}`} style={{ display: "flex", gap: 12, color: theme.ink, textDecoration: "none", fontSize: 14 }}><Mail size={18} color={theme.accent} />{store.contactEmail}</a>}{avgRating != null && <div style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 13 }}>{Array.from({length:5}).map((_,i)=><Star key={i} size={14} color={theme.accent} fill={i < Math.round(avgRating) ? "currentColor" : "none"}/>)} {avgRating.toFixed(1)} guest rating</div>}</div></div></div></section>
  </>);
}
