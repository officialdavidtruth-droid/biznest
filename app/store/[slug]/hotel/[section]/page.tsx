...
export default async function HotelSectionPage({ params }: { params: Promise<{ slug: string; section: string }> }) {
  ...
  const gallery = Array.from(new Set([store.bannerUrl, store.storyImage, ...items.map((i) => i.image)].filter(Boolean) as string[])).slice(0, 12);
  ...
  if (section === "gallery") return shell(<section style={{ padding: "110px 28px 130px" }}><div style={{ maxWidth: 1240, margin: "0 auto" }}>{sectionTitle(theme, "Visual narrative", "See the place before you arrive.")}<div style={{ display: "grid", gridTemplateColumns: "1.4fr .8fr .8fr", gridAutoRows: 260, gap: 12, marginTop: 55 }}>{gallery.length ? gallery.map((image, i) => <div key={image} style={{ gridRow: i === 0 ? "span 2" : undefined, background: `url(${image}) center/cover` }} />) : <p style={{ color: muted }}>Gallery images will appear here once they are added.</p>}</div></div></section>);
