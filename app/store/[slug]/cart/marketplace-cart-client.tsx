...
// Marketplace Hub design tokens (see lib/template-themes.ts MARKETPLACE) —
// mirrors violet-cart-client.tsx's structure but recolored/reshaped so it
// matches the square cards, blue/orange accents, and boxy borders used
// across the rest of a Marketplace-templated store.

export function MarketplaceCartClient({ slug }: { slug: string }) {
  ...
  return (
    <div style={{ maxWidth: 1120, margin: "0 auto", fontFamily: MARKETPLACE.font, color: MARKETPLACE.ink, fontSize: 13 }} className="px-6 py-8">
      <div style={{ borderBottom: `1px solid ${MARKETPLACE.border}` }} className="mb-6 flex items-baseline justify-between pb-3">
        <h1 style={{ fontSize: 22, fontWeight: 700 }}>Your Cart</h1>
        <span style={{ opacity: 0.6, fontSize: 12 }}>{itemCount} item{itemCount !== 1 ? "s" : ""}</span>
      </div>

      {cartItems.length === 0 ? (
        <div style={{ background: "#fff", border: `1px solid ${MARKETPLACE.border}` }} className="p-10 text-center">
          <p style={{ opacity: 0.7, fontSize: 13 }}>Your cart is empty.</p>
          <Link href={`/store/${slug}`} style={{ color: MARKETPLACE.blue, fontSize: 13 }} className="mt-3 inline-block font-semibold no-underline hover:opacity-80">
            Continue shopping →
          </Link>
        </div>
      ) : (
        <div className="grid gap-6 lg:grid-cols-12">
          <div className="flex flex-col gap-3 lg:col-span-8">
            {cartItems.map((item) => (
              <div
                key={item.productId}
                style={{ background: "#fff", border: `1px solid ${MARKETPLACE.border}` }}
                className="flex flex-col gap-4 p-4 sm:flex-row"
              >
                <div style={{ background: "#f2f2f2" }} className="h-32 w-full flex-shrink-0 overflow-hidden sm:h-auto sm:w-32">
                  {item.image ? (
                ...
                <div className="flex flex-grow flex-col justify-between">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span style={{ fontSize: 13.5, fontWeight: 700, color: MARKETPLACE.price }}>
                      {item.currency} {(item.price * item.quantity).toLocaleString()}
                    </span>
                  </div>
                  <div style={{ borderTop: `1px solid ${MARKETPLACE.border}` }} className="mt-3 flex items-center justify-between pt-3">
                    <div style={{ border: `1px solid ${MARKETPLACE.border}` }} className="flex items-center overflow-hidden">
