      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "36px 28px 80px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 40 }}>
          ...
          <div>
            ...
            {service.isBookable ? (
              <BookingWidget storeSlug={slug} serviceId={service.id} serviceName={service.name} price={Number(service.price)} currency={service.currency} durationMins={service.durationMins} totalUnits={service.totalUnits} accent={accent} ink={ink} bg={bg} radius={radius} startOpen />
            ) : (
